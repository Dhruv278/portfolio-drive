    ...
    <>
      {HILLS.map((h, i) => (
        <mesh key={i} position={poseAt(h.t, h.lateral).position} scale={[1, h.h / h.r, 1]} castShadow receiveShadow>
          ...
          <meshStandardMaterial map={map} normalMap={s.grassNor} roughness={1} metalness={0} color={h.dark ? '#344943' : '#435b50'} />
        </mesh>
      ))}
    </>
  )
      ...
      {BILLBOARDS.map((b, i) => {
        ...
        return (
          <group key={i} position={pose.position} quaternion={pose.quaternion}>
            ...
            <Cyl r={0.14} h={3.6} c={COLORS.ink} x={1.8} seg={6} />
            <Box w={5.2} h={2.8} d={0.2} c="#1b2326" y={3.4} />
            <Box w={4.2} h={0.4} d={0.05} c="#d6ed83" y={5.2} z={0.12} />
            <Box w={3.4} h={0.25} d={0.05} c="#edf0e9" x={-0.4} y={4.5} z={0.12} />
            <Box w={2.6} h={0.25} d={0.05} c="#a5b3b5" x={-0.8} y={4.0} z={0.12} />
          </group>
        )
      })}
    </>
