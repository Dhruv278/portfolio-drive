...
// The assistant's dialog. Loaded on demand by AskBot.tsx, so pages that never open it do not ship
// or hydrate this code.
export type Msg = { role: 'user' | 'bot'; text: string; sources?: string[]; sig?: string; error?: boolean }
type Turn = { role: 'user' | 'assistant'; content: string; sig?: string }
type Props = { page: 'track' | 'other'; onClose: () => void }
...
export function AskBotPanel({ page, onClose }: Props) {
  ...
  return (
    <div ref={dialog} className="dbot" role="dialog" aria-modal="true" aria-labelledby="dbot-title" data-testid="dbot">
      <header className="dbot-head">
        ...
        <button type="button" className="dbot-close" onClick={onClose} aria-label="Close">
