...
Prompts live in an append-only registry with versions and restore. Patient identifiers are scrubbed before text reaches a model. Tenant scoping and rate limits hold on every route. The medications prompt is replayed against known records before it ships and compared with the previous version, and I am extending that harness to every extraction stage.
