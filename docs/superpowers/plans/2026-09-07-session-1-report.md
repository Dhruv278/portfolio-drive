...
## Addendum: one dark theme, and a loading screen for the drive (12 September 2026)

The owner reported the "old light 3D UI" after going to the resume and back. Reproduced on the live site with a real browser: the drive stays dark after that navigation; what is light is the resume page, and the writing pages, which still carried the paper theme. So the paper theme is gone: the legacy tokens (`--paper`, `--ink`, `--cobalt` and friends) are aliases of the dark set at the root, the hard paper drop shadows are deleted, the drive's colour overrides are unscoped and shared by every page, the resume is a navy card with amber mono section headings (print stays black on white through the print rules), the writing pages sit on the navy body, and the resume top bar carries the 2D / 3D switch with neither side active. The resume header no longer lowercases the availability line.
...
## Addendum: the audit's fixes (12 September 2026)

Everything in the audit report that did not need facts only the owner holds is done. Content: the replay-evaluation claim now matches the owner's own account (medications prompt, harness being extended), "three roles since 2023" replaces "three and a half years" and the recruiter FAQ names the gap, the skills heading no longer claims every skill shipped, the six platforms carry one honest framing line, the QA-tracker and tooling lines appear once each, two achievements that restated experience bullets are gone, three UK spellings are fixed, the write-up describes the drive rather than a vanished home page and quotes the night build's frame rate, both write-ups are published with honest reading times, and the PDF is regenerated from the builder with the new header and all six platforms, still two pages. The assistant: first-person salary rule, the "not in my record" sentence only on its own, mandatory citation when correcting a premise, no contractions, the visitor's language, a wider override list, the email appended to any decline that forgot it, a "Numbers on the site" section so the bot knows the figures the pages show, the phone number removed from its record, and a signature on every answer so a forged earlier turn is dropped before the model sees it. Layout: the paragraph rules that flattened the type hierarchy on both pages are demoted with :where, the skip link lands on the home content, the assistant button sits above the drive hint on desktop and above the bottom sheet on phones, live regions announce the checkpoint name only, every animation stops under reduced motion, touch targets reach 44 px, the resume measures 760 px at 15.5 px inside a main landmark, skills stack on phones, the assistant's field grows, source chips are plain text where they cannot scroll, the drive hero text has a shadow over the lit garage, unlit dots have more contrast, the finish button's pulse is gated like the switch, the bottom HUD and the assistant button wait for the scene while the loader shows. Engineering: the project clips render posters and attach video only near the viewport and never on narrow screens, the drive has an error boundary and a context-loss fallback, drive links no longer prefetch the scene bundle, integrated GPUs render at device pixel ratio 1 with 1024 shadow maps and a once-rendered contact shadow, sky and star geometry and the light-pool texture are disposed on remount, the assistant has upstream and client timeouts, the origin check accepts the request host so a custom domain works, questions need three characters, and the site has metadataBase, canonical links, an Open Graph image, structured data, robots, a sitemap, a navy theme color, security headers, long cache headers for textures and media, no powered-by header and a Node engine pin. Escape no longer leaves the drive.
